  ...
  #            "beta_mot.csv", "beta_tail.csv",
  #            "nma_mot.csv", "nma_tail.csv")
  files <- c("emb_mot.csv", "beta_mot.csv",
             "nma_mot.csv")
  # files <- c("emb_tail.csv", "beta_tail.csv",
  #            "nma_tail.csv")
  # files <- c("emb_tail.csv", "beta_tail.csv",
  #            "nma_tail.csv", "2b_tail.csv",
  #            "nmb_tail.csv", "2a_tail.csv")
  
  
  opar <- par(mfrow = c (3,2), mar = c(0,4,1.5,1), oma = c(3, 3, 0, 0))
  ...
  for (input in files){
    ...
    legend = paste(c("R² = ", round(robR, digits = 3),
      ...
      if (grepl('mot', input)){
        ...
        # title (main="Motor")
        legend("topright",inset=c(0, 0.02), legend=toupper(ni), text.font = 2, bty = "n", cex = 2)
        }
      ...
      if (grepl('beta', input)){
        ...
        legend("topright", legend=expression(beta), inset=c(0.03, -0.03),text.font = 2, bty = "n", cex = 3)
        }
      }
    if (grepl('nma', input)){
    ...
    if (grepl('beta', input)){
      if (grepl('mot', input)){
        legend("topright", legend=expression(beta), inset=c(0, 0.02),text.font = 2, bty = "n", cex = 3)
      }
    }
    if (grepl('nma', input)){
      axis(1,  at=c(0.01,0.1,1,10,100,1000,10000, 100000), labels= c("0.01", "0.1", "1", "10", "100",  expression(paste("10"^"3")), expression(paste("10"^"4")), expression(paste("10"^"5"))), cex.axis = 1.2)
  ...
  # mtext('Mass (kg)', side = 1, outer = TRUE, line = 2 )
  mtext('dS', side = 2, outer = TRUE, line = -27)
  ...
  jpeg(filename = "dn_sup.jpeg",
       width = 18, height = 25, units = "cm",
       quality = 75, res = 200)
  
  
  opar <- par(mfrow = c (3,2), mar = c(0,1,1.5,1), oma = c(3, 3, 0, 0))
  ...
  for (input in files){
    ...
    if (grepl('mot', input)){
      ...
      legend("topright", legend=toupper(ni), text.font = 2, bty = "n", cex = 2)}
      # axis(2,  at=c( 0,0.2,0.4,0.6,0.8), labels= c(  0,0.2,0.4,0.6,0.8))
      axis(2, at= seq(0,0.08, by = 0.01), labels = seq(0,0.08, by = 0.01), cex.axis = 1.2)
      axis(2, at= seq(0,0.8, by = 0.01), labels = F)
